    ...
    if get_data(file_path) is not None:
        ...
        for i in range(nrows):
            ...
            for j in range(ncols):
                # 获取当前列中文标题
                title_de = str(row_0[j]).encode(encoding="utf-8")
                title_cn = title_de.split(' '.encode(encoding ="utf-8"))
                # 获取单元格的值
                tmp[title_cn] = sheet.row_values(i)[j]
            result["photo"].append(tmp)
        json_data = json.dumps(result, indent=4, sort_keys=True).encode(encoding="utf-8")
